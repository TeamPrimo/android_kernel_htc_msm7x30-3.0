...
/*#define PRIMOC_SDMC_CD_N_TO_SYS PM8058_GPIO_PM_TO_SYS(PRIMOC_GPIO_SDMC_CD_N)*/
...
#if 0
static int __init primoc_disablesdcard_setup(char *str)
{
...
__setup("board_primoc.disable_sdcard=", primoc_disablesdcard_setup);
...
static uint32_t primoc_sdslot_switchvdd(struct device *dev, unsigned int vdd)
{
	int i;
...
static unsigned int primoc_sdslot_status(struct device *dev)
{
	unsigned int status;

	status = (unsigned int) gpio_get_value(PRIMOC_SDMC_CD_N_TO_SYS);

	return (!status);
}

#define PRIMOC_MMC_VDD		(MMC_VDD_28_29 | MMC_VDD_29_30)

static unsigned int primoc_sdslot_type = MMC_TYPE_SD;

static struct mmc_platform_data primoc_sdslot_data = {
	.ocr_mask	= PRIMOC_MMC_VDD,
	.status_irq	= MSM_GPIO_TO_INT(PRIMOC_SDMC_CD_N_TO_SYS),
	.status		= primoc_sdslot_status,
	.translate_vdd	= primoc_sdslot_switchvdd,
	.slot_type	= &primoc_sdslot_type,
	.dat0_gpio	= 63,
};

static unsigned int primoc_emmcslot_type = MMC_TYPE_MMC;
static struct mmc_platform_data primoc_movinand_data = {
	.ocr_mask	=  PRIMOC_MMC_VDD,
	.slot_type	= &primoc_emmcslot_type,
...
static int primoc_wifi_cd;	/* WiFi virtual 'card detect' status */
...
int __init primoc_init_mmc(unsigned int sys_rev)
{
	...
	*/
...
#if 0
	msm_add_sdcc(2, &primoc_movinand_data, 0, 0);
#endif
	...
	if (opt_disable_sdcard) {
		printk(KERN_INFO "primoc: SD-Card interface disabled\n");
		goto done;
	}
	...
	set_irq_wake(MSM_GPIO_TO_INT(PRIMOC_SDMC_CD_N_TO_SYS), 1);

	msm_add_sdcc(4, &primoc_sdslot_data,
			MSM_GPIO_TO_INT(PRIMOC_SDMC_CD_N_TO_SYS),
			IORESOURCE_IRQ_LOWEDGE | IORESOURCE_IRQ_HIGHEDGE);
done:
#endif

	/* reset eMMC for write protection test */
	gpio_set_value(PRIMOC_GPIO_EMMC_RST, 0);	/* this should not work!!! */
	udelay(100);
